...
% capacity analysis is not performed for the whole country
...
% toevaluate=toevaluate(17:21);
toevaluate={'lombardia'};

% toevaluate={'lombardia','piemonte','liguria','emilia','lazio','marche','toscana','veneto','valledaosta','friuli'};
% toevaluate={'valledaosta'};
...
parfor key=1:length(toevaluate)
    ...
    disp('pass1')
    Copy_of_func_replace_string(sprintf('Models/last_model_t_undetected_%s.def',getfield(CN,toevaluate{key})),...
        sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),'line1',line1);
    disp('pass2')
    Copy_of_func_replace_string(sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),...
        sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),'line2',line2);
    disp('pass3')
    Copy_of_func_replace_string(sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),...
        sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),'init_x1',init_x1);
    disp('pass4')
    Copy_of_func_replace_string(sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),...
        sprintf('Models/covid_19_Tanmay_2_%s.def',getfield(CN,toevaluate{key})),'init_x2',init_x2);
    
    %loop file modification
    
    Copy_of_func_replace_string(sprintf('Models/last_model_t_undetected_loop_%s.def',getfield(CN,toevaluate{key})),...
        sprintf('Models/last_model_t_loop_2_%s.def',getfield(CN,toevaluate{key})),'line1',line1);
    
    Copy_of_func_replace_string(sprintf('Models/last_model_t_loop_2_%s.def',getfield(CN,toevaluate{key})),...
    ...
    deffile=sprintf('covid_19_Tanmay_2_%s',getfield(CN,toevaluate{key}));
    ...
    ar=exp_loop(datatoread,deffile,loopfile,cityname,...
