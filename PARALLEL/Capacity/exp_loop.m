...
if missing>0
    vectlim=num2((missing+3):length(num2),2);
    hosplim=num3((missing+3):length(num3),2);
    %     vectlim=num2((missing+4):length(num2),2);
    %     hosplim=num3((missing+4):length(num3),2);
else
    vectlim=num2(3:length(num2),2);
    hosplim=num3(3:length(num3),2);
...
%%% alpha perturbation for reviewers' comments
% arSetPars('alpha', log10(0.4),2,1,log10(0.01),log10(0.5));
arSetPars('alpha', log10(0.2),2,1,log10(0.01),log10(0.5));
%%%
...
for i=seq
    ...
    Copy_of_func_replace_string(sprintf('Models/last_model_t_loop_2_%s.def',...
        cityname),...
    ...
    for j=2:15
        ...
        %disp(rl)
        Copy_of_func_replace_string(sprintf('Models/last_model_t_loop_3_%s.def',...
            cityname),...
    ...
    [num,txt,raw]=xlsread(strcat(home_dir,'/',datatoread));
    minshift=0;
    ...
    arSetPars('rho', log10(1/5),1,1,log10(0.01),log10(0.9));
    arSetPars('thet', log10(0.2),1,1,log10(0.01),log10(0.7));
    arSetPars('delta', log10(0.1),1,1,log10(0.1),log10(0.9));
...
save([sprintf('area_specific_%s.mat',cityname)],'savr0','savpars','savinit','savchi2','toplot','savr0per')
