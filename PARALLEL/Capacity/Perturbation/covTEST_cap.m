function dydt = covTEST_cap(t,x, alpha, bet, delta, ki,...
            r1, r10, r3, r4, r5, r6, r7, r8, rho, thet,...
            undFIX,undVAR, pop, hlim,icum)
%%% eqs from /home/abp19/covid-19/COVID/capacity_IXD/Models
...
dydt = zeros(N,1);
%%
dydt(1) = -r1*(x(3)+x(4)+ki*x(14)+bet*(x(5)+x(6)+x(15)+x(16)))*x(1)/pop;  % sus _a1 x1
dydt(2) = r1*(x(3)+x(4)+ki*x(14)+bet*(x(5)+x(6)+x(15)+x(16)))*x(1)/pop -(r3/(r3*5.2 -1))*x(2);  % exp x2
dydt(3) = (1-alpha)*(r3/(r3*5.2 -1))*x(2) - r3*x(3);  % CI_a1 x3
dydt(4) = alpha*(r3/(r3*5.2 -1))*x(2) - x(4)*(2*r3*r4/(2*r4+r3));  % CR x4
dydt(5) = ((1-undFIX)/(1-alpha))*rho*r3*x(3)-r6*x(5)*(1-(exp(x(7)+x(8)-hlim)^10/(1+exp(x(7)+x(8)-hlim)^10))^10) - r6*x(5)*((exp(x(7)+x(8)-hlim)^10/(1+exp(x(7)+x(8)-hlim)^10))^10);  % IH x5
...
dydt(11) = r4*x(16)+x(6)*r4 + x(8)*r5 + x(10)*r8;  %   RZ x11
...
dydt(16) = ((undFIX - undVAR)/(1-alpha))*r3*x(3) -r4*x(16);     %// Ixd x16
