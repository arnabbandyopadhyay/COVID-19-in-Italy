...
library(latex2exp)
    ...
    for (ii in 1:pertnum) {
        ...
        ###LATE TESTING
        tmpplo <- readMat(paste0(thisishere,'/LateTest_res/',ii,'_',rr,'_02.mat'))
        ...
        ###EARLY TESTING
        tmpplo <- readMat(paste0(thisishere,'/EarlyTest_res/',ii,'_',rr,'_02.mat'))
...
#     caprefe <- cbind(tt, rtmp, rep('MaxCap',length(hoscap)), hoscapM )
#     caprefesd <- cbind(tt, rtmp, rep('MaxCap',length(hoscap)), hoscapsds )
#     
#     deadcaprefe <- cbind(tt, rtmp, rep('MaxCap',length(hosEA)), deadcapM )
#     deadcaprefesd <- cbind(tt, rtmp, rep('MaxCap',length(hosEA)), deadcapsd )
...
xlab('') + ylab('Hospitalized') + ggtitle('B')
...
geom_text(collseldead, mapping=aes(label = paste0(value,'%'), y=where, x=as.Date(95, origin='2020-02-24',by='days',format='%Y-%m-%d') ,col=Model), show.legend = FALSE) +
scale_x_date(date_breaks = "15 days",date_labels = "%b %e") +
...
#pdf('TestMAXCap_02.pdf',h=11,w=11)
...
MaxCapPlot <- grid.arrange(ppfithos,ppfitdead,ncol=1)

print('1 of 3 done; end of First Part ...')
...
library(latex2exp)
    ...
    for (ii in 1:pertnum) {
        ...
        ###LATE TESTING
        tmpplo <- readMat(paste0(thisishere,'/LateTest_res/',ii,'_',rr,'_cap_02.mat'))
        AAd<-tmpplo$x[,5]+tmpplo$x[,6]+tmpplo$x[,15]+tmpplo$x[,16]
        BBd<-tmpplo$x[,7]+tmpplo$x[,8]
        CCd<-tmpplo$x[,9]+tmpplo$x[,10]
        DDd<-tmpplo$x[,12]
        ...
        ###EARLY TESTING
        tmpplo <- readMat(paste0(thisishere,'/EarlyTest_res/',ii,'_',rr,'_cap_02.mat'))
        AAc<-tmpplo$x[,5]+tmpplo$x[,6]+tmpplo$x[,15]+tmpplo$x[,16]
        BBc<-tmpplo$x[,7]+tmpplo$x[,8]
        CCc<-tmpplo$x[,9]+tmpplo$x[,10]
        DDc<-tmpplo$x[,12]
...
updown <- c(-500,-500,-1500,1500,-200,-200,-500,-500)
ppfitdead <- ggplot(DD, aes(x=as.Date(Time, origin='2020-02-24',by='days',format='%Y-%m-%d'), y=value, col=Model)) +
geom_line() +
geom_ribbon(aes(ymin=value-sd, ymax=value+sd, fill=Model),col=NA, alpha=0.5) +
geom_point(dmseld, mapping=aes(x=as.Date(Time, origin='2020-02-24',by='days'), y=value), col='black', size=0.1) +
...
CapPlot <- grid.arrange(ppfithos,ppfitdead,ncol=1)

#pdf('TestCap02_fig.pdf',h=11,w=11)
#grid.arrange(CapPlot,MaxCapPlot,ncol=1)
#dev.off()

ggsave(
  'TestCap02_fig.pdf',
  plot = grid.arrange(CapPlot,MaxCapPlot,ncol=1),
  width = 11,
  height = 11,
  units = "in",
  dpi = 300
