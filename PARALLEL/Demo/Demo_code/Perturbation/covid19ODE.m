...
dydt = zeros(N,1);
%%
dydt(1) = -r1*(x(3)+x(4)+ki*x(14)+bet*(x(5)+x(6)))*x(1)/pop;  % sus _a1 x1
dydt(2) = r1*(x(3)+x(4)+ki*x(14)+bet*(x(5)+x(6)))*x(1)/pop -(r3/(r3*5.2 -1))*x(2);  % exp x2
dydt(3) = (1-alpha)*(r3/(r3*5.2 -1))*x(2) - r3*x(3);  % CI_a1 x3
dydt(4) = alpha*(r3/(r3*5.2 -1))*x(2) - x(4)*(2*r3*r4/(2*r4+r3));  % CR x4
dydt(5) = ((1-und)/(1-alpha))*rho*r3*x(3)-r6*x(5);  % IH x5
...
dydt(13) = x(4)*(2*r3*r4/(2*r4+r3))+r4*x(14)+r4*x(6);  %  RX x13
