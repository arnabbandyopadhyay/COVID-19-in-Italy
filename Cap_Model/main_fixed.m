...
parfor key=1:length(toevaluate)
    ...
    %% Load the parameters generated with the Capacity Model (exp_cap.m)
    % pre-generated files available
    
    savpars=load(sprintf('/Users/Arnab/Desktop/code/Cap_Model/cap_per_results/final_params_%s.mat',getfield(CN,toevaluate{key})));
    ...
    for pper=1:size(savpars.comb,2)
        fixpars=savpars.comb(pper).params;
        ar=fixedloop(datatoread,deffile,loopfile,cityname,...
            missing, fixpars,pper);
